...
@Component({
  ...
  template: `
    <div class="flex h-screen bg-background overflow-hidden">
      ...
      <!-- Sidebar -->
      <aside class="fixed inset-y-0 left-0 z-50 transform transition-all duration-300 ease-in-out md:relative md:translate-x-0 md:flex flex-shrink-0 bg-sidebar border-r border-sidebar-border"
             [class.-translate-x-full]="!isMobileMenuOpen()"
             [class.translate-x-0]="isMobileMenuOpen()"
             [class.w-64]="!isDesktopSidebarCollapsed()"
             [class.w-20]="isDesktopSidebarCollapsed()">
        <app-sidebar [collapsed]="isDesktopSidebarCollapsed()" (toggle)="toggleDesktopSidebar()"></app-sidebar>
      </aside>

      <!-- Main Content Wrapper -->
      <div class="flex flex-col flex-1 min-w-0 overflow-hidden">
        <!-- Mobile Header -->
        <header class="md:hidden flex items-center justify-between px-4 py-3 bg-sidebar border-b border-sidebar-border">
